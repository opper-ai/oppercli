...
func (c *HelpCommand) Execute(ctx context.Context, client *opperai.Client) error {
	...
	fmt.Println("\n  indexes:")
	fmt.Println("    list [filter]              List indexes, optionally filtering by name")
	fmt.Println("    create <name>              Create a new index")
	fmt.Println("    delete <name>              Delete an index")
	fmt.Println("    get <name>                 Get index details")
	fmt.Println("    query <name> <query> [filter_json]  Query an index")
	fmt.Println("    add <name> <key> <content> [metadata_json]  Add content to an index")
	fmt.Println("    upload <name> <file_path>  Upload and index a file (PDF, CSV, TXT)")
